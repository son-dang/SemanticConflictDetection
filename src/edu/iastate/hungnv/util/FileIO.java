...
public class FileIO {
	...
	/**
	 * Returns 1 if file1.timestamp > file2.timestamp; -1 if file1.timestamp < file2.timestamp; 0 otherwise.
	 */
	...
	public static void copyFileOrFolder(String sourceFile, String targetFile) {
	    ...
	    if (sourceLocation.isDirectory()) {
	        ...
	        for (int i = 0; i < children.length; i++) {
	            copyFileOrFolder(sourceLocation + "/" + children[i], targetLocation + "/" + children[i]);
	        }
	    } else {
	 ...
	 * Line starts from 1.
	 */
	public static int getLineFromOffsetInFile(String file, int offset) {
