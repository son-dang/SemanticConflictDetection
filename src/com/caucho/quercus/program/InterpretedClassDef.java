...
public class InterpretedClassDef extends ClassDef
    ...
    private InterpretedClassDef _aggregateClass = null;

    public InterpretedClassDef getAggregateClass() {
        return this._aggregateClass;
    }

    public void setAggregateClass(InterpretedClassDef aggregateClass) {
    ...
    public /*final*/ HashMap<String, AbstractFunction> _functionMap = new HashMap<String, AbstractFunction>();
    public /*final*/ HashMap<StringValue, FieldEntry> _fieldMap = new LinkedHashMap<StringValue, FieldEntry>();
    public /*final*/ HashMap<String, StaticFieldEntry> _staticFieldMap = new LinkedHashMap<String, StaticFieldEntry>();
    public /*final*/ HashMap<String, Expr> _constMap = new HashMap<String, Expr>();
    ...
    public void initClass(QuercusClass cl) {
        ...
        for (Map.Entry<String, StaticFieldEntry> entry : _staticFieldMap.entrySet()) {
            ...
            cl.addStaticFieldExpr(className, entry.getKey(), field.getValue(), this.getConstraint());
        }
     ...
     */
    public void initInstance(Env env, Value value, Constraint constraint) {
        ObjectValue object = (ObjectValue) value;

        // CODE ADDED BY SON
        // when init an instance, default value of fields can be a MultiValue
        // alter the default init instance to act as expected
        if (Env_.INSTRUMENT) {
            InterpretedClassDef temp = this;
            
            while (temp != null) {
                Constraint classConstraint = temp.getConstraint();
                if (classConstraint == null)
                	classConstraint = Constraint.TRUE;
                env.getEnv_().enterNewScope(classConstraint);
                for (Map.Entry<StringValue, FieldEntry> entry : temp._fieldMap.entrySet()) {
                    FieldEntry fieldEntry = entry.getValue();
                    Value oldValue = object.getFieldForInitializer(env, entry.getKey());
                    Value newValue = temp._fieldMap.get(entry.getKey()).getValue().eval(env);
                    newValue = env.getEnv_().addScopedValue(oldValue, newValue);
            ...
            for (Map.Entry<StringValue, FieldEntry> entry : _fieldMap.entrySet()) {
                ...
                Value fieldValue = fieldEntry.getValue().eval(env).copy();
                object.initField(entry.getKey(),
                        fieldValue,
                        fieldEntry.getVisibility());
            }
