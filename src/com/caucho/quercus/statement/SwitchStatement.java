...
public class SwitchStatement extends Statement {
  ...
  public String toString(){
      ...
      retValue.append("switch (").append(this._value.toString()).append("){\r\n");
      for(int i=0; i<this._cases.length; i++){
          for(Expr _case : this._cases[i]){
              retValue.append("case ").append(_case.toString()).append(":\r\n");
          }
          retValue.append(this._blocks[i].toString());
      }
      
      if (this._defaultBlock != null){
          retValue.append("default:\r\n");
          retValue.append(this._defaultBlock.toString());
      }
  ...
  public boolean equal(Statement stm){
      if (!(stm instanceof SwitchStatement))
          return false;
      
      SwitchStatement swStm = (SwitchStatement)stm;
      return swStm._value.toString().equals(this._value.toString());
  }
  
/*  @Override
  public ArrayList<Statement> alignStatement(ArrayList<Statement> statements){
      HashMap<String, Expr> allCases = new HashMap<String, Expr>();
      for(Statement stm : statements){
          SwitchStatement swStm = (SwitchStatement)stm;
          for(Expr[] _cases : swStm._cases){
              for (Expr _case : _cases)
                  allCases.put(_case.toString(), _case);
          }
      }
