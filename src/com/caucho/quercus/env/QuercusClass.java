...
public class QuercusClass extends NullValue {
	...
	// CODE ADDED BY SON
    private Constraint _constraint= Constraint.TRUE;
    ...
    private QuercusClass _aggregateClass = null;
    public QuercusClass getAggregateClass(){
        return this._aggregateClass;
    }
    public void setAggregateClass(QuercusClass aggregateClass){
   ...
   */
  public void addStaticFieldExpr(String className, String name, Expr value, Constraint constraint)
  {
  ...
  public void init(Env env)
  {
    ...
    for (Map.Entry<String,ArrayList<StaticField>> map
      ...
      if (env.isInitializedClass(map.getKey()) && !Env_.INSTRUMENT)
        continue;
...
//    StringValue staticName = _staticFieldNameMap.get(name);
      StringValue staticName = null;
      QuercusClass qClass = this;
      while(qClass != null){
          staticName = qClass._staticFieldNameMap.get(name);
          if (staticName != null)
              break;
          qClass = qClass.getAggregateClass();
      }
...
/*       for (int i = 0; i < _initializers.size(); i++) {
          _initializers.get(i).initInstance(env, obj, Constraint.TRUE);
        }*/
      
      QuercusClass qc = this;
      while(qc != null){
          for(int i=0; i<qc._initializers.size(); i++)
              qc._initializers.get(i).initInstance(env, obj, _constraint);
    ...
    /*  Set<String> classesName = new HashSet<String>();
      QuercusClass qc = this;
      while(qc != null){
          for (ClassDef _cd : qc._classDefList)
              classesName.add(_cd.getName());
          qc = qc.getAggregateClass();
      }
      ...
      for(String className : classesName){
          ...
          while(qc != null){
              ...
              for(int i=0; i< qc._initializers.size(); i++){
                  ...
                  if (((InterpretedClassDef)qc._initializers.get(i)).getName().equals(className)){
                      qc._initializers.get(i).initInstance(env, obj, Constraint.TRUE);
                      foundClass = true;
                      break;
                  }
              }
      ...
      if (_isJavaWrapper) {
        ...
        if (Env_.INSTRUMENT){
	        ...
	        while (qc != null) {
	            ...
	            Constraint andConstraint = Constraint.createAndConstraint(constraint, aggregateConstraint);
	            ...
	            for(int i=0; i<args.length; i++){
	                ...
	                tempSwitch.addCases(MultiValue.flatten(args[i]));
	                tempSwitch.addCase(new Case(constraint, args[i]));
	                tempArgs[i] = MultiValue.createSwitchValue(tempSwitch);
	            }
	            
	            AbstractFunction fun = qc.findConstructor();
	            
	            if (fun != null) {
	                fun.callMethod(env, this, objectValue, tempArgs);
	            } else {
	                //  if expr
	            }
