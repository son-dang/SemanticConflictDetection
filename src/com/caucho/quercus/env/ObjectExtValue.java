  ...
  implements Serializable
{
  ...
  public final Value getField(Env env, StringValue name)
  {
    ...
    public Value getFieldForInitializer(Env env, StringValue name) {
        Value returnValue = getFieldExt(env, name);
...
// END OF ADDED CODE
  ...
  {
      if (Env_.INSTRUMENT){
          QuercusClass qc = this._quercusClass;
          ...
          while (qc != null){
              try{
                  ...
                  AbstractFunction fun = qc.getMethodMap().get(methodName, hash);
                  Value []tempArgs = new Value[args.length];
                  for(int i=0; i<args.length; i++){
                      Switch tempSwitch = new Switch();
                      if (args[i].isMultiValue())
                  ...
                  Value value = fun.callMethod(env, qc, this, tempArgs);
                  env.getEnv_().exitScope();
                  if (value.isMultiValue()){
                      Switch valueSwitch = MultiValue.flatten(value);
                      for(Case valueCase : valueSwitch.getCases()){
                          Constraint valueConstraint = valueCase.getConstraint();
                          andConstraint = Constraint.createAndConstraint(constraint, valueConstraint);
                          if (andConstraint.isSatisfiable())
                              _switch.addCase(new Case(andConstraint, valueCase.getValue()));
                      }
                  }
                  else{
